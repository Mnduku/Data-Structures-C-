...
namespace cop4530 {
    ...
    class List {
       ...
       public:
          ...
          void reverse()
          {
            auto itr1 = begin();                   //Makes an iterator for the list and iterates through it
            List<T> temp;                         //Pushing each element to the front thus reversing the order
            while (itr1.current->next != nullptr)
            {
              temp.push_front(itr1.current->data);
              itr1++;
             }
              *this = temp;
          }
    ...
    typename List<T>::const_iterator i1 = rhs.begin();
    typename List<T>::const_iterator i2 = lhs.begin();
    for (; i1 != lhs.end(); i1++)
    {
